...
::ImportImageFilter()
{
  ...
  m_ImageContainerManageMemory = false;
  m_Size = 0;
}
...
::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "Import buffer size: " << m_Size << std::endl;
  os << indent << "ImageContainer manages memory: " << ( m_ImageContainerManageMemory ? "true" : "false" ) << std::endl;
...
::SetImportPointer(TPixel *ptr, SizeValueType num, bool LetImageContainerManageMemory)
{
  ...
  m_ImageContainerManageMemory = LetImageContainerManageMemory;
  m_Size = num;
}
